...
impl UnixTime {
    ...
    // ASTRID SPIKE PATCH: rustls's "std" feature references UnixTime::now()
    // from ticketer/DefaultTimeProvider — server-side paths a client with a
    // custom TimeProvider never runs. Provide a panicking stub so the crate
    // compiles on wasm32-unknown-unknown (which has no wall clock).
    #[cfg(all(
        feature = "std",
        target_family = "wasm",
        target_os = "unknown",
    ...
    pub fn now() -> Self {
        unimplemented!("no wall clock on wasm32-unknown-unknown; use a custom rustls TimeProvider")
    }
